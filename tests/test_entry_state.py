...
def test_recently_read(auth_config):
    ...
    mock_post = respx.post(
        "https://api.feedbin.com/v2/recently_read_entries.json"
    ).mock(return_value=httpx.Response(200, json=[6]))
    result = runner.invoke(app, ["recently-read", "create", "6"])
    assert result.exit_code == 0
    assert json.loads(mock_post.calls[0].request.content) == {
        "recently_read_entries": [6]
    }
