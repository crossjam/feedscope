    ...
    try:
        with get_client() as client:
            ...
            if response.status_code != 200:
                typer.echo(f"Error fetching saved searches: {response.status_code}", err=True)
                raise typer.Exit(1)
            ...
            if json_output:
                ...
                for search in searches:
                    typer.echo(f"[{search['id']}] {search['name']} - Query: {search['query']}")
...
def get_search(
    ...
    include_entries: Annotated[bool, typer.Option(help="Include full entry objects")] = False,
    ...
    try:
        with get_client() as client:
            ...
            if response.status_code == 201:
                typer.echo("✅ Saved search created successfully.", color=typer.colors.GREEN)
                if json_output:
                    typer.echo(json.dumps(response.json(), indent=2))
            else:
                typer.echo(f"Error creating saved search: {response.status_code}", err=True)
                if json_output:
...
def update_search(
    ...
    try:
        with get_client() as client:
            ...
            if response.status_code == 200:
                typer.echo("✅ Saved search updated successfully.", color=typer.colors.GREEN)
                if json_output:
                    typer.echo(json.dumps(response.json(), indent=2))
                ...
                typer.echo(f"Error updating saved search: {response.status_code}", err=True)
                raise typer.Exit(1)
    ...
    try:
        with get_client() as client:
            ...
            if response.status_code == 204:
                typer.echo("✅ Saved search deleted successfully.", color=typer.colors.GREEN)
            elif response.status_code == 403:
                typer.echo("Forbidden. You may not own this saved search.", err=True)
                raise typer.Exit(1)
            else:
                typer.echo(f"Error deleting saved search: {response.status_code}", err=True)
                raise typer.Exit(1)
