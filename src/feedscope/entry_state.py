...
def mark_read(
    ...
    _manage_entries_state(ctx, "unread_entries", "DELETE", entry_ids, "unread_entries", json_output)
...
def mark_unread(
    ...
    _manage_entries_state(ctx, "unread_entries", "POST", entry_ids, "unread_entries", json_output)
...
def star_entries(
    ...
    """Star entries."""
    _manage_entries_state(ctx, "starred_entries", "POST", entry_ids, "starred_entries", json_output)
...
def unstar_entries(
    ...
    """Unstar entries."""
    _manage_entries_state(ctx, "starred_entries", "DELETE", entry_ids, "starred_entries", json_output)

# Updated
@updated_app.command(name="list")
def list_updated(
    ctx: typer.Context,
    since: Annotated[Optional[datetime], typer.Option(help="Get entries updated after this timestamp")] = None,
    include_diff: Annotated[bool, typer.Option(help="Fetch details including content diff")] = False,
    json_output: Annotated[bool, typer.Option("--json", help="Output raw JSON")] = False,
):
    ...
    if include_diff:
        ...
        try:
            with get_client() as client:
                 ...
                 if response.status_code != 200:
                     typer.echo(f"Error fetching updated IDs: {response.status_code}", err=True)
                     raise typer.Exit(1)
                 ids = response.json()
            ...
            if len(ids) > 100:
                 typer.echo(f"Warning: Only showing first 100 of {len(ids)} updated entries.", err=True)
...
def mark_updated_read(
    ...
    _manage_entries_state(ctx, "updated_entries", "DELETE", entry_ids, "updated_entries", json_output)
...
def list_recently_read(
    ...
    fetch_and_display_entries(
        ctx,
        "https://api.feedbin.com/v2/recently_read_entries.json",
        {},
        json_output
    )
...
def create_recently_read(
    ctx: typer.Context,
    entry_ids: Annotated[List[int], typer.Argument(help="Entry IDs to add to recently read")],
    json_output: Annotated[bool, typer.Option("--json", help="Output raw JSON")] = False,
):
    """Add entries to recently read."""
    _manage_entries_state(ctx, "recently_read_entries", "POST", entry_ids, "recently_read_entries", json_output)