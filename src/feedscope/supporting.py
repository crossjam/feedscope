        ...
        with get_client() as client:
            ...
            if response.status_code == 200:
                ...
                if json_output:
                    ...
                    for imp in imports:
                        typer.echo(f"ID: {imp['id']}, Complete: {imp['complete']}, Created: {imp['created_at']}")
            else:
        ...
        with get_client() as client:
            ...
            if response.status_code == 200:
                ...
                if json_output:
                    ...
                    typer.echo(f"Import {imp['id']} Status: {'Complete' if imp['complete'] else 'Pending'}")
...
def create_import(
    ctx: typer.Context,
    file_path: Annotated[Path, typer.Argument(help="Path to OPML file", exists=True, readable=True)],
    json_output: Annotated[bool, typer.Option("--json", help="Output raw JSON")] = False,
):
        ...
        with get_client() as client:
            ...
            if response.status_code == 201:
                ...
                if json_output:
                 ...
                 typer.echo(f"Error: {response.status_code}", err=True)
                 if json_output: typer.echo(response.text)
                 raise typer.Exit(1)
    except httpx.RequestError as e:
        typer.echo(f"❌ Network error: {e}", color=typer.colors.RED)
        raise typer.Exit(1)
...
def extract_command(
    ...
    if not username or not secret:
        typer.echo("❌ Extraction credentials not found in config (extract.username, extract.secret).", color=typer.colors.RED)
        raise typer.Exit(1)
